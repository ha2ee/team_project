   ...
   //댓글 리스트 조회
      public ArrayList<CommentVO> listComment(int b_idx) {
         ...
         try {
            ...
            String sql = "SELECT c.*, COALESCE(m.mem_nick, t.tr_name) AS name "
                  + "FROM tblComment c LEFT JOIN ys_member m ON c.id = m.mem_id "
                  + "LEFT JOIN member_trainer t ON c.id = t.tr_id WHERE pseq = ? "
                  + "ORDER BY seq ASC";
